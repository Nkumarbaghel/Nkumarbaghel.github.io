(() => {
const REPO='Nkumarbaghel/cgneelbaghel', PATH='posts.js', BRANCH='main', OWNER='Nkumarbaghel';
...
async function verifyOwnerAndRepo(){
  const meR=await fetch('https://api.github.com/user',{headers:{Authorization:'Bearer '+token,Accept:'application/vnd.github+json','X-GitHub-Api-Version':'2022-11-28'}});
  const me=await meR.json();
  if(!meR.ok) throw new Error('Token invalid है या GitHub ने access अस्वीकार किया।');
  if(String(me.login||'').toLowerCase()!==OWNER.toLowerCase()) throw new Error('⛔ यह GitHub account Admin नहीं है। केवल '+OWNER+' account को अनुमति है।');
  const repoR=await fetch('https://api.github.com/repos/'+REPO,{headers:{Authorization:'Bearer '+token,Accept:'application/vnd.github+json','X-GitHub-Api-Version':'2022-11-28'}});
  const repo=await repoR.json();
  if(!repoR.ok) throw new Error('Repository access नहीं मिला।');
  if(repo.permissions && repo.permissions.push!==true) throw new Error('⛔ इस token में repository Contents write permission नहीं है।');
  return me;
}
